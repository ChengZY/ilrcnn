...
def make_data_loader(cfg, is_train=True, is_distributed=False, start_iter=0, external_proposal=False, compression_not_shuffle=False):
    ...
    for dataset in datasets:
        ...
        collator = BatchCollator(cfg.DATALOADER.SIZE_DIVISIBILITY, cfg) # 用于把一个batch的数据转换成ImageList格式
...
def make_dummy_data_loader(cfg, is_train=True, is_distributed=False, start_iter=0, external_proposal=False):
    num_gpus = get_world_size()

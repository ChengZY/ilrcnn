...
if __name__ == '__main__':
    ...
    if args.group > 0: # F # If not train from the first group, We should load the weights here
    ...
    for key, value in dict(fasterRCNN.named_parameters()).items():  # since we froze some layers
        if value.requires_grad:
            ...
            if 'RCNN_rpn.RPN_cls_score' in key:  # Record the parameter position of RPN_cls_score
                rpn_cls_params_index.append(ith) # [2,3]
            if 'RCNN_base' in key:
                base_net_params_index.append(ith) # [6,7,...,86,87,88]
    ...
    group_cls, group_cls_arr, group_merged_arr = ciod_old_and_new(
        cfg.NUM_CLASSES, cfg.CIOD.GROUPS, cfg.CIOD.DISTILL_GROUP) # 20, 4, False
    # group_cls: [1,6,11,16,21]; 5
    # group_cls_arr: [tensor([0,1,2,3,4,5]), tesnor([0,6,7,8,9,10]), ..., tesnor([0,16,17,18,19,20])] 4
    # group_merged_arr: [[tensor([0])], [tensor([0,1,2,3,4,5])], [tensor([0,..,10])],[tensor([0,...,15])]] 4
    # Train ALL groups, or just ONE group
    start_group, end_group = (0, cfg.CIOD.GROUPS) if args.group == -1 else (args.group, args.group + 1)
    # start_group: 0; end_group: 4
    ...
    for group in trange(start_group, end_group, desc="Group", leave=True):
        ...
        change_require_gradient(b_fasterRCNN, False) # reuqire_grad set to False
        ...
        for epoch in trange(cfg.TRAIN.MAX_EPOCH, desc="Epoch", leave=True):
            ...
            for i in trange(iters_per_epoch, desc="Iter", leave=True):
                ...
                im_path = list(data[4])
                # im_info: (N,3) 3 -> H,W,resize_scale
                # cls_prob: (N,nr_rois,21), cls_score: (N*nr_rois,21) score->softmax->prob
                # bbox_raw: (N*nr_rois,21*4)
                # bbox_pred: (N*nr_rois,4), bbox_raw根据rois_label只输出该labels对应的box的coord预测
                # rpn_feature: (N,512,38,57) # feature map
                ...
                rois_target, rois_inside_ws, rois_outside_ws \
                    = fasterRCNN(im_data, im_info, gt_boxes, num_boxes, distill=False,
                ...
                # rois_label, pooled_feat, cls_score, \
                # rpn_loss_cls, rpn_loss_bbox, RCNN_loss_cls, RCNN_loss_bbox,
                # rois_target, rois_inside_ws, rois_outside_ws = out
                # set_trace()
                # cls_prob: (N,nr_roi,21); rois: (N,nr_roi,5) 5->(batch_index,x1,y1,x2,y2)
                RCNN_loss_bbox_distill = Variable(torch.FloatTensor([0.0])).cuda()
                loss_frcn_cls_old = Variable(torch.FloatTensor([0.0])).cuda()
                # RCNN_loss_bbox_distill = torch.FloatTensor([0.0])
                # loss_frcn_cls_old = torch.FloatTensor([0.0])
                ...
                if (0 != group) and (cfg.CIOD.SWITCH_DO_IN_RPN or cfg.CIOD.SWITCH_DO_IN_FRCN):
                    ...
                    b_rois_target, b_rois_inside_ws, b_rois_outside_ws \
                        = b_fasterRCNN(im_data, im_info, gt_boxes, num_boxes, distill=True,
                        rois_in=rois, rpn_loss_cls_in=rpn_loss_cls, rpn_loss_bbox_in=rpn_loss_bbox, 
                        rpn_label_in=rpn_label, rpn_feature_in=rpn_feature, rpn_cls_score_in=rpn_cls_score,
                        rois_label_in=rois_label, rois_target_in=rois_target, rois_inside_ws_in=rois_inside_ws, 
                        rois_outside_ws_in=rois_outside_ws)
                    if args.vis:
                        ##### show bbox #####
                    ...
                    if cfg.CIOD.SWITCH_DO_IN_FRCN:
                        ...
                        loss_frcn_cls_old = Variable(torch.FloatTensor([0.0])).cuda() # 0
                        # set_trace()
                        for index_old in group_merged_arr[group]: # 遍历之前的old class
                            # b_cls_score/cls_score: size(81,21); 
                            # index_select表示从对应class维度上取对每个roi的pred score
                            label_old = heat_exp(b_cls_score.index_select(1, index_old), cfg.CIOD.TEMPERATURE) # !softmax with temperature
                        ...
                        # 采样gt label=0的背景roi，获取对应的cls score的值，对其的score distribution进行mimic
                        if cfg.CIOD.DISTILL_BACKGROUND: # False
                            zero_label_mask = (rois_label == 0).nonzero().squeeze()
                            label_zero_f = b_cls_score.index_select(0, zero_label_mask)# FIXME: 应该使用b_cls_score???
                        ...
                        # 对预测的box coord进行distill
                        if cfg.CIOD.DISTILL_BOUNDINGBOX and not args.class_agnostic: # DISTILL_BOUNDINGBOX=False, args.class_agnostic=False
                ...
                loss = rpn_loss_cls.mean() + rpn_loss_bbox.mean() \
                       + RCNN_loss_cls.mean() + RCNN_loss_bbox.mean() + RCNN_loss_bbox_distill.mean()
                ...
                if tot_step % cfg.TRAIN.DISPLAY == 0:
                    ...
                    # set_trace() 
                    loss_box_dist = RCNN_loss_bbox_distill.mean().data[0]
                    loss_cls_dist = loss_frcn_cls_old.mean().data[0]
                    fg_cnt = torch.sum(rois_label.data.ne(0))
                    bg_cnt = rois_label.data.numel() - fg_cnt
                    # if (0 != group) and (cfg.CIOD.SWITCH_DO_IN_RPN or cfg.CIOD.SWITCH_DO_IN_FRCN):
                    #     set_trace()
                    tqdm.write("[S{} G{}] lr: {:.2}, loss: {:.4}, fg/bg=({}/{})\n"
                               "\t\t\trpn_cls: {:.4}, rpn_box: {:.4}, rcnn_cls: {:.4}, rcnn_box {:.4}, box_dist {:.4}, cls_dist {:.4}".format(
                        args.session, group, lr, loss_temp, fg_cnt, bg_cnt,
                        loss_rpn_cls, loss_rpn_box, loss_rcnn_cls, loss_rcnn_box, loss_box_dist, loss_cls_dist))
        ...
        if cfg.CIOD.REPRESENTATION: # Fasle
            if args.save_without_repr:  # False # We can save weights before representation learning

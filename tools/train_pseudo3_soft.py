...
"""
apply pseudo annotation with soft classification label
注意：需要改动Source_model.yaml中增加LABEL_TYPE: "soft"字段
"""
...
def do_train(model_source, model_target, data_loader, optimizer, scheduler, checkpointer_source, checkpointer_target,
    ...
    average_pseudo_softlabel_loss = 0
    average_classifier_loss = 0
    ...
    for iteration, (images, targets, _, idx) in enumerate(data_loader, start_iter):
        ...
        for gt, pred in zip(targets, src_pred):
            ...
            pred_softlabels = pred.get_field("softlabels")
            ...
            if cfg_source.MODEL.LABEL_TYPE == "soft":
                gt.extra_fields['softlabels'] = torch.cat(
                    [torch.zeros((gt.extra_fields['labels'].shape[0], pred_softlabels.shape[1]), dtype=torch.float).cuda(), 
                    pred_softlabels], dim=0)
        ...
        faster_rcnn_losses = sum(loss_dict_target[x] for x in faster_rcnn_key)
        # print(cfg_source.MODEL.SOFT_WEIGHT)
        pseudo_softlabel_losses = loss_dict_target['loss_softclassifier'] * cfg_source.MODEL.SOFT_WEIGHT
        classifier_losses = loss_dict_target['loss_classifier'] # only for show to compare with softclassifier
        """
        ...
        losses = faster_rcnn_losses + pseudo_softlabel_losses
        ...
        if (iteration - 1) > 0:
            ...
            average_classifier_loss = (average_classifier_loss * (iteration - 1) + classifier_losses) /iteration
            average_faster_rcnn_loss = (average_faster_rcnn_loss * (iteration - 1) + faster_rcnn_losses) /iteration
            average_pseudo_softlabel_loss = (average_pseudo_softlabel_loss * (iteration - 1) + pseudo_softlabel_losses) /iteration
        else:
            # average_distillation_loss = distillation_losses
            average_classifier_loss = classifier_losses
            average_faster_rcnn_loss = faster_rcnn_losses
            average_pseudo_softlabel_loss = pseudo_softlabel_losses
        ...
        if iteration % 20 == 0 or iteration == max_iter:
            ...
            summary_writer.add_scalar('pseudo_softlabel_losses_avg', average_pseudo_softlabel_loss, iteration)
            summary_writer.add_scalar('classifier_losses_avg', average_classifier_loss, iteration)

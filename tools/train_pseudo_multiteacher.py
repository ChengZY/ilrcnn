...
def do_train(model_source, model_target, data_loader, optimizer, scheduler, checkpointer_source, checkpointer_target,
    ...
    for iteration, (images, targets, _, idx, trans_info) in enumerate(data_loader, start_iter):

        # TODO: trans_info: 存储该image执行的transformer的参数，用来save to npy时的还原
        # trans_info: (list(color_jitter, resize, hor_flip, totensor, norm))
        ...
        trans_info = [_iter for _iter in trans_info]
        idxs = list(idx)
        im_ids = [data_loader.dataset.get_img_id(x) for x in idxs]
        img_size = [(_sz[1], _sz[0]) for _sz in images.image_sizes] # [(w,h)]
        # source model prediction
        src_pred = model_source(images)[0]

        # 需要保存回的原box信息
        with torch.no_grad():
            # detach from graph to create a clean BoxList
            clean_src_pred = []
            for gt, pred in zip(targets, src_pred):
                tmp = BoxList(pred.bbox.data, pred.size, mode="xyxy")
                before_num = len(tmp)
                # from ipdb import set_trace; set_trace()
                tmp_score = pred.get_field("scores").data
                tmp_label = pred.get_field("labels").data
                tmp.add_field("scores", tmp_score)
                tmp.add_field("labels", tmp_label)
                # start to filter
                keep = torch.nonzero(tmp_score > cfg_target.MODEL.PSEUDO_CONF_THRESH).squeeze(1)
                tmp = tmp[keep]
                if len(tmp) > 0:
                    iou_matrix = boxlist_iou(tmp, gt)
                    max_iou = iou_matrix.max(dim=1)[0]
                    # set_trace()
                    keep2 = torch.nonzero(max_iou < cfg_target.MODEL.PSEUDO_IOU_THRESH).squeeze(1)
                    tmp = tmp[keep2]
                after_num = len(tmp)
                # print("Filter Box Number: {} -> {}".format(before_num, after_num))
                clean_src_pred.append(tmp)
                # clean_src_pred = [BoxList(pred.bbox.data, pred.size, mode="xyxy") for pred in src_pred]
            result_origin = [trans_reverse(pred, info) for pred, info in zip(clean_src_pred, trans_info)]


        # TODO: 返回到原图变换前的pred box状态，存储
        if iteration % cfg_target.MODEL.SAVE_PRED_PERIOD == 0:
            # 不需要一个iter全部过一遍dataloader，边ema边保存吧，每次就取前5个prediction融合即可。
            # 存储结构：source/img_id_pred.pt
            for i, im_id in enumerate(im_ids):
                # 读取该img_id_pred.pt {iteration: list(pred_box)}
                pt_path = os.path.join(cfg_source.OUTPUT_DIR, "{}_pred.pt".format(im_id))
                if os.path.isfile(pt_path):
                    pt_dict = torch.load(pt_path, map_location='cpu')
                else:
                    pt_dict = OrderedDict()
                pt_dict[iteration] = result_origin[i]

                torch.save(pt_dict, pt_path)


        # TODO: read npy file -> transform box -> box fusion
        # 统一从文件中读，全部执行trans_reverse
        for i, im_id in enumerate(im_ids):
            pt_path = os.path.join(cfg_source.OUTPUT_DIR, "{}_pred.pt".format(im_id))
            ...
            # from ipdb import set_trace; set_trace()
            select_iter = pt_keys[-cfg_target.MODEL.ENSEMBLE_NUM:]
            # box list
            ens_boxlist = [pt_dict[x] for x in pt_keys] # pt_dict[x]: BoxList
            t_info = trans_info[i]
            _sz = img_size[i]
            t_info[1] = _sz
            # 修改resize的shape为当前图片的size
            trans_ens_boxlist = [trans_reverse(boxlist, t_info) for boxlist in ens_boxlist]

            # TODO: WEIGHT BOX FUSION together as pseudo box
            # TODO: 根据enns_boxlist里的score和label进行合并bounding box
            # merged box fusion
            # refer to https://github.com/ZFTurbo/Weighted-Boxes-Fusion/blob/master/ensemble_boxes/ensemble_boxes_wbf.py
            inp_boxes_list = copy.deepcopy([x.bbox.tolist() for x in trans_ens_boxlist])
            inp_score_list = copy.deepcopy([x.get_field("scores").tolist() for x in trans_ens_boxlist])
            inp_label_list = copy.deepcopy([x.get_field("labels").tolist() for x in trans_ens_boxlist])
            if args.vis and len(select_iter) >= 2: # cfg_target.MODEL.ENSEMBLE_NUM:
                print("=> Fusion Iteration: ", select_iter)
                before_ens_img = visual_multi_iter_box(images.tensors[i], inp_boxes_list, inp_score_list, inp_label_list, select_iter)
            # ens_boxes, ens_scores, ens_labels -> np.array (nr_pred,4), (nr_pred,1), (nr_pred,1)
            ens_boxes, ens_scores, ens_labels = weighted_boxes_fusion(inp_boxes_list, inp_score_list, inp_label_list)

            gt = targets[i]
            before_num = gt.bbox.shape[0]
            # from ipdb import set_trace; set_trace()
            merge_boxes = torch.cat([gt.bbox, torch.from_numpy(ens_boxes).to(device).to(torch.float32)], dim=0)
            gt.bbox = merge_boxes
            gt.add_field("scores", torch.cat(
                [torch.ones_like(gt.extra_fields['labels'], dtype=torch.float32), 
                torch.from_numpy(ens_scores).to(device).to(torch.float32)], dim=0))
            gt.add_field("is_gt", torch.cat([torch.ones_like(gt.extra_fields['labels']), torch.zeros((ens_labels.shape[0],), dtype=torch.long).to(device)], dim=0))
            gt.extra_fields['labels'] = torch.cat([gt.extra_fields['labels'], torch.from_numpy(ens_labels).to(device).to(torch.long)], dim=0)
            ...
            print("Box Number (GT -> GT+Fusion -> GT+Fusion(threshold)): {} -> {} -> {}/{}".format(before_num, mid_num, after_num, targets[i].bbox.shape[0]))

            # visualize multiple iteration
            if args.vis and len(select_iter) >= 2: # cfg_target.MODEL.ENSEMBLE_NUM:
                # visualize multiple iteration prediction
                after_ens_img = visual_ens_box(images.tensors[i], targets[i])
                merge_img = np.hstack([before_ens_img, after_ens_img])
                cv2.imshow("Before_ens(No GT): After_ens(With GT)", merge_img)
